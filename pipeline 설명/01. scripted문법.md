...
<h1> jenkins pipeline은 **scripted, declarative**라는 2가지 문법을 지원하며 해당 챕터에서는 **scripted**를 소개한다</h1>
 일반적으로 declarative를 사용한다. 해당 챕터는 가볍게만 보자.

<span style="color:blue"> scripted 문법의 전체 구조를 한번 훑고 넘어가자.</span>

### 다음과 같이 최상위 태그가 node라면 declarative 문법이라는걸 의미한다<br>안녕
```groovy
node {
