# 01. scripted문법
jenkins pipeline은 scripted, declarative라는 2가지 문법을 지원하며 해당 챕터에서는 scripted를 소개한다<br>일반적으로 declarative를 사용한다. 해당 챕터는 가볍게 scripted 문법의 전체 구조를 한번 훑고 넘어가자. 다음과 같이 최상위 태그가 node라면 scripted 문법이라는걸 의미한다
```groovy
node {
