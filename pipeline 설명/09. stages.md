...
## 목적 : 다른 태그들은 모두 편의를 위한 옵션에 불과하며 <br>실질적인 기능을 수행하는 모든 구문은 여기서 작성한다.
## stages는 필수 태그이며 하위태그로 1개 이상의 stage("")를 사용한다. 괄호 내부에는 내가 구별하기 위한 명칭을 대입하면 된다. 해당 명칭은 빌드할때 다음과 같이 표시되며, 각 stage마다 로그를 확인할 수 있다.
![stage](/images/stage.png)
## stage 하위 태그로는 다음의 4가지 중 1가지를 사용하여야 한다. 순차실행(steps), 순차실행 세분화(stages), 병렬 동시 실행(parallel), 경우의 수별로 순차 또는 병렬실행(matrix)을 하나씩 살펴보겠다.
...
#### 1-2) input : 빌드 실행도중 값을 입력받는다. 아직은 필요성이 없어 생략한다. (stage 하위 태그)

#### 1-3) script : scripted pipeline block 을 가져와서 groovy 를 사용가능. if - else, try - catch 도 사용가능하다. (steps 하위 태그)
```groovy
def gv;
...
### 2. stages : stage가 더 작은 stage들로 나눠지게 해준다. 순회적으로 사용이 가능하다.
```groovy
pipeline {
    agent none
    stages {
...
### 3. parallel : 블럭 안에 stage 블럭들을 넣으면 병렬 수행이 가능하다.
```groovy
pipeline {
    agent any
    stages {
...
### 4. matrix : 3가지 경우의 수 * 3가지 경우의 수 를 배치하면 9가지 경우의 수를 모두 수행해준다. 예시를 보면 쉽게 이해가 간다.
```groovy
matrix {
