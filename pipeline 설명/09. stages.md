...
#### 1-2) input
빌드 실행도중 값을 입력받는다. 아직은 필요성이 없어 생략한다. 사용형태만 알아보자 (stage 하위 태그)
```groovy
pipeline {
    agent any
    stages {
...
#### 1-3) script
scripted pipeline block 을 가져와서 groovy 를 사용가능. if - else, try - catch 도 사용가능하다. (steps 하위 태그)
```groovy
def gv;
...
### 4. matrix : 3가지 경우의 수 * 3가지 경우의 수를 배치하면 9가지 경우의 수를 모두 수행해준다. 예시를 보면 쉽게 이해가 간다.
```groovy
matrix {
